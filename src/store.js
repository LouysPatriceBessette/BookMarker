...
let reducer = (state, action) => {
    ...
    if (action.type === "DELETE link") {
        ...
        newState.categories[newState.activeCat].content = newState.categories[
            newState.activeCat
        ].content.filter(item => {
    ...
    if (action.type === "click count") {
        newState.links[action.linkID].clickCount
            .reverse()
            .push(action.newClickDate);
    }
